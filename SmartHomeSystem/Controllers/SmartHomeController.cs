...
namespace SmartHomeSystem.Controllers
{
    ...
    {
        private readonly InMemoryDatabaseService _dbService;
        ...
        private readonly Timer _sensorReadingTimer;
        ...
        {
            Console.WriteLine("Initializing Smart Home Controller...");
            ...
            Console.WriteLine("Smart Home Controller initialized successfully.");
        }
        ...
        public void Start()
        {
            if (_isRunning)
            {
                Console.WriteLine("Smart Home Controller is already running.");
                return;
            }
            
            Console.WriteLine("Starting Smart Home Controller...");
            ...
            _isRunning = true;
            Console.WriteLine("Smart Home Controller started.");
        }
        ...
        public void Stop()
        {
            if (!_isRunning)
            {
                Console.WriteLine("Smart Home Controller is not running.");
                return;
            }
            
            Console.WriteLine("Stopping Smart Home Controller...");
            ...
            _isRunning = false;
            Console.WriteLine("Smart Home Controller stopped.");
        }
        ...
        public void AddDevice(SmartDevice device, int roomID, string deviceName)
        {
            ...
            if (!string.IsNullOrEmpty(deviceName))
            {
                ...
                Console.WriteLine($"Device alias '{deviceName}' registered for device {device.DeviceID}");
            }
        }
        ...
        private void CheckSchedules(object state)
        {
            try
            {
                ...
                Console.WriteLine($"Checking schedules at {now}...");
                ...
                {
                    Console.WriteLine("No active schedules found.");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error checking schedules: {ex.Message}");
            }
        }
        ...
        private void ExecuteScheduledAction(Schedule schedule)
        {
            try
            {
                ...
                if (device == null)
                {
                    Console.WriteLine($"Error executing schedule {schedule.ScheduleID}: Device {schedule.DeviceID} not found");
                    return;
                }
                
                var (actionName, parameters) = schedule.ParseAction();
                
                Console.WriteLine($"Executing scheduled action: {actionName} for device {device.DeviceID} ({device.GetType().Name})");
            ...
            {
                Console.WriteLine($"Error executing schedule {schedule.ScheduleID}: {ex.Message}");
            }
        }
        ...
        private void TakeSensorReadings(object state)
        {
            try
            {
                Console.WriteLine("Taking sensor readings...");
            ...
            {
                Console.WriteLine($"Error taking sensor readings: {ex.Message}");
            }
        }
        ...
        {
            Console.WriteLine($"Motion detected in {e.Location} at {e.DetectionTime}");
            ...
            foreach (var light in lights)
            {
                ...
                {
                    light.TurnOn();
                    Console.WriteLine($"Automatically turned on {light.GetType().Name} in {e.Location} due to motion detection");
                }
            }
            ...
            foreach (var camera in cameras)
            {
                if (camera.GetStatus() && !camera.IsRecording)
                {
                    ...
                    Console.WriteLine($"Automatically started recording on camera in {e.Location} due to motion detection");
                }
            }
        }
        ...
        /// </summary>
        private void HandleTemperatureThreshold(object sender, TemperatureThresholdEventArgs e)
        {
            Console.WriteLine($"Temperature threshold reached in {e.Location}: {e.Temperature}°C ({e.ThresholdType})");
            ...
            foreach (var thermostat in thermostats)
            {
                ...
                {
                    if (e.ThresholdType == TemperatureThresholdType.High)
                    {
                        // If temperature is too high, set mode to Cool
                        thermostat.SetMode("Cool");
                        Console.WriteLine($"Automatically set thermostat in {e.Location} to Cool mode due to high temperature");
                    }
                    else if (e.ThresholdType == TemperatureThresholdType.Low)
                    {
                        // If temperature is too low, set mode to Heat
                        thermostat.SetMode("Heat");
                        Console.WriteLine($"Automatically set thermostat in {e.Location} to Heat mode due to low temperature");
                    }
                }
            }
        }
